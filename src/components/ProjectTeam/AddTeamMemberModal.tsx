...
import { addStudentToProject } from '../../lib/firebase';
...
export const AddTeamMemberModal: React.FC<AddTeamMemberModalProps> = ({
  ...
  const availableStudents = students.filter(student => !currentStudentIds.includes(student.id));
  ...
  const handleStudentToggle = (studentId: string) => {
    setSelectedStudentIds(prev => {
      if (prev.includes(studentId)) {
        ...
        if (currentStudents.length + prev.length >= maxStudents) {
          return prev;
        }
