...
import { useProjectEvaluation, EvaluationCriteria } from '../../hooks/useProjectEvaluation';
...
export const EvaluationForm: React.FC<EvaluationFormProps> = ({ projectId, onSaved }) => {
  ...
  const getCriterionIcon = (name: string) => {
    switch (name) {
      case 'درجة الإنجاز': return <Target className="w-5 h-5" />;
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-6 text-center">
          <div className="w-32 h-32 bg-blue-50 rounded-full flex flex-col items-center justify-center mx-auto">
            <span className="text-3xl font-bold text-blue-600">{currentEvaluation.percentage}%</span>
            <span className="text-sm text-blue-600">الدرجة النهائية</span>
          </div>
        </div>
