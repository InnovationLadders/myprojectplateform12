...
export const EvaluationSummary: React.FC<EvaluationSummaryProps> = ({ 
  ...
  const getCriterionIcon = (name: string) => {
    switch (name) {
      case 'درجة الإنجاز': return <Target className="w-4 h-4" />;
    ...
    >
      ...
      <div className="flex items-center justify-center mb-6">
        <div className="w-32 h-32 bg-blue-50 rounded-full flex flex-col items-center justify-center">
          <span className="text-3xl font-bold text-blue-600">{evaluation.percentage}%</span>
          <span className="text-sm text-blue-600">{getGradeText(evaluation.percentage)}</span>
        </div>
      </div>
      ...
      {evaluation.feedback && (
        <div className="mb-4">
          ...
          <div className="bg-gray-50 p-4 rounded-xl">
            <p className="text-gray-700 whitespace-pre-line">{evaluation.feedback}</p>
          </div>
        </div>
      )}
